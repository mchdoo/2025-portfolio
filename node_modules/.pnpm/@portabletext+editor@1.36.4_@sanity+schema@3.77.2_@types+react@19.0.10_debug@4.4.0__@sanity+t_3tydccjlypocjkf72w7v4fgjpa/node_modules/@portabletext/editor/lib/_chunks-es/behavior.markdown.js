...
function createMarkdownBehaviors(config) {
  const automaticBlockquoteOnSpace = defineBehavior({
    ...
    guard: ({
      ...
      const text = event.data.getData("text/plain"), hrRegExp = /^(---)$|(___)$|(\*\*\*)$/gm, hrCharacters = text.match(hrRegExp)?.[0], hrObject = config.horizontalRuleObject?.(snapshot.context), focusBlock = getFocusBlock(snapshot);
