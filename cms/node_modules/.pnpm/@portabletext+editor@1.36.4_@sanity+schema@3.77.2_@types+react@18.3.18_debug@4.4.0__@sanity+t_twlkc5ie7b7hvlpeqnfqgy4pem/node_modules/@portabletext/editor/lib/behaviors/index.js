...
function createCodeEditorBehaviors(config) {
  return [defineBehavior({
    ...
    guard: ({
      ...
      const isMoveUpShortcut = isHotkey(config.moveBlockUpShortcut, event.keyboardEvent), firstBlock = getFirstBlock(snapshot), selectedBlocks = getSelectedBlocks(snapshot), blocksAbove = firstBlock?.node._key !== selectedBlocks[0]?.node._key;
    ...
    guard: ({
      ...
      const isMoveDownShortcut = isHotkey(config.moveBlockDownShortcut, event.keyboardEvent), lastBlock = getLastBlock(snapshot), selectedBlocks = getSelectedBlocks(snapshot), blocksBelow = lastBlock?.node._key !== selectedBlocks[selectedBlocks.length - 1]?.node._key;
...
function createLinkBehaviors(config) {
  const pasteLinkOnSelection = defineBehavior({
    ...
    guard: ({
      ...
      const selectionCollapsed = isSelectionCollapsed(snapshot), text = event.data.getData("text/plain"), url = looksLikeUrl(text) ? text : void 0, annotation = url !== void 0 ? config.linkAnnotation?.({
        url,
